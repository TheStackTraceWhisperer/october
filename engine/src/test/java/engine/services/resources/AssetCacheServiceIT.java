...
public class AssetCacheServiceIT extends EngineTestHarness {
  ...
  void testAssetLoadingAndCaching() {
    ...
    Shader shader1 =
        assetCacheService.loadShader("test-shader", vertexShaderPath, fragmentShaderPath);
    ...
    assertSame(
        texture1, texture2, "Loading the same texture handle should return the cached instance.");
    assertSame(
        shader1, shader2, "Loading the same shader handle should return the cached instance.");
    assertSame(
        audioBuffer1,
        audioBuffer2,
        "Loading the same audio handle should return the cached instance.");
  }
}
