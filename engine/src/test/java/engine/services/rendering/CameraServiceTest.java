...
class CameraServiceTest {
  ...
  void testSetPositionUpdatesViewMatrix() {
    ...
    Matrix4f expected =
        new Matrix4f()
            .lookAt(newPosition, new Vector3f(10.0f, 5.0f, 19.0f), new Vector3f(0.0f, 1.0f, 0.0f));
  ...
  void testSetPerspectiveUpdatesProjectionMatrix() {
    ...
    Matrix4f projection = cameraService.getProjectionMatrix();
    Matrix4f expected =
        new Matrix4f().perspective((float) Math.toRadians(fov), aspectRatio, near, far);
    assertEquals(expected, projection);
  }
  ...
  void testResizeUpdatesOrthographicProjection() {
    ...
    Matrix4f expected =
        new Matrix4f().ortho(-expectedHalfWidth, expectedHalfWidth, -4.5f, 4.5f, -1.0f, 100.0f);
    assertEquals(expected, projection);
  }
}
