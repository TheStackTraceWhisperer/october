...
public class AudioServiceIT extends EngineTestHarness {
  ...
  void testAudioPlaybackLifecycle() {
    ...
    AudioBuffer soundBuffer =
        assetCacheService.loadAudioBuffer("test-sound", "audio/test-sound.ogg");
  ...
  void testMasterVolumeControl() {
    ...
    assertEquals(
        newVolume, audioService.getMasterVolume(), 0.001f, "Master volume should be updated");
  ...
  void testMasterVolumeClampedAtZeroWhenNegative() {
    ...
    assertEquals(
        0.0f, audioService.getMasterVolume(), 0.001f, "Negative master volume should clamp to 0.0");
  }
  ...
  void testSourceVolumeClampAndStates() {
    ...
    AudioBuffer soundBuffer =
        assetCacheService.loadAudioBuffer("test-sound-2", "audio/test-sound.ogg");
  ...
  void testAudioBufferCloseThenGetIdThrows() {
    ...
    assertThrows(
        ...
        "getBufferId after close should throw");
  }
}
