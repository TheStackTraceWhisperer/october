...
public class AudioSystemIT extends EngineTestHarness {
  ...
  void testSystem_cleansUpSourceWhenComponentIsRemoved() throws Exception {
    ...
    assertTrue(
        musicSourceMap.isEmpty(),
        "Internal music source should be removed after component is removed.");
  ...
  void testMusicFadeInThenFadeOutStopsPlayback() throws Exception {
    ...
    while (music.fadingIn && safety-- > 0) {
      ...
      Thread.sleep(1);
      assertTrue(
          music.currentVolume >= lastVolume - 0.001f, "Volume should not decrease during fade-in.");
      lastVolume = music.currentVolume;
    }
    assertFalse(music.fadingIn, "Fade-in should complete.");
    assertEquals(
        music.baseVolume, music.currentVolume, 0.1f, "Volume should be at base after fade-in.");
    ...
    while (music.fadingOut && safety-- > 0) {
      ...
      Thread.sleep(1);
      assertTrue(
          music.currentVolume <= lastVolume + 0.001f,
          "Volume should not increase during fade-out.");
      lastVolume = music.currentVolume;
    }
    ...
    AudioSource source = musicSourceMap.get(entity);
    assertTrue(
        source.isStopped() || !music.isPlaying,
        "Source should be stopped or flag cleared after fade-out.");
  }
  ...
  void testPlaySoundEffectHelper_setsVolumeAndCleansUpAfterStop() throws Exception {
    ...
    assertNull(
        worldService.getComponent(entity, SoundEffectComponent.class),
        "SoundEffectComponent should be removed after stop");
    assertFalse(
        getInternalEffectSourceMap().containsKey(entity),
        "Effect source should be removed from internal map after cleanup");
  }

  /**
   * Uses reflection to access the private source map for verification. This is an acceptable
   * testing practice for verifying internal state management.
   */
  @SuppressWarnings("unchecked")
  private Map<Integer, AudioSource> getInternalMusicSourceMap()
  ...
  @SuppressWarnings("unchecked")
  private Map<Integer, AudioSource> getInternalEffectSourceMap()
      throws NoSuchFieldException, IllegalAccessException {
