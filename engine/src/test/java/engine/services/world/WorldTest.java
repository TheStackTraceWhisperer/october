...
class WorldTest {
  ...
  void addAndGetComponent_managesComponentLifecycle() {
    ...
    // Then
    assertTrue(
        world.hasComponent(entity, ComponentA.class), "World should report entity has component.");
    assertSame(
        component,
        world.getComponent(entity, ComponentA.class),
        "Retrieved component should be the same instance.");
    ...
    // Then
    assertFalse(
        world.hasComponent(entity, ComponentA.class), "World should report component is removed.");
    assertNull(
        world.getComponent(entity, ComponentA.class),
        "getComponent should return null for removed component.");
  }
  ...
  void getEntitiesWith_returnsAllEntitiesWithAtLeastTheGivenComponents() {
    ...
    assertEquals(
        1, entitiesWithAB.size(), "Should find 1 entity with both ComponentA and ComponentB.");
  ...
  void destroyEntity_removesAllComponentsAndSignature() {
    ...
    // Then
    assertFalse(
        world.hasComponent(entity, ComponentA.class),
        "ComponentA should be gone after entity destruction.");
    assertFalse(
        world.hasComponent(entity, ComponentB.class),
        "ComponentB should be gone after entity destruction.");
    assertTrue(
        world.getEntitiesWith(ComponentA.class, ComponentB.class).isEmpty(),
        "No entities should match signature after destruction.");
  }
}
